...
We then assess the data per lab in terms of sample size and CDI score (vocabulary size, for consistency between language zones). This Figure shows raw vocabulary scores (mean and distribution) for each lab by age and language zone.
...
plot.pearson.nae <- data.nae %>%
  ...
  labs(x="IDS preference", y = "Standardized CDI score", title = "North American English") +
  geom_point() +
...
plot.pearson.uk.18 <- data.uk.18 %>%
  ggplot(aes(x = IDS_pref,
             y = vocab_nwords)) +
  labs(x="IDS preference", y="Vocabulary size (in words)", title="UK English 18 months") +
  geom_point() +
  geom_smooth(method = lm) +
...
plot.pearson.uk.24 <- data.uk.24 %>%
  ggplot(aes(x = IDS_pref,
             y = vocab_nwords)) +
  labs(x="IDS preference", y="Vocabulary size (in words)", title="UK English 24 months") +
  geom_point() +
  geom_smooth(method = lm) +
  annotate("text", x = .8, y = 150, parse = T, size = 4,
           label = paste(cor_text, cor_value, sep = "~"))
...
#TODO: There is still more clean-up needed on these plots
...
plot.age_group <- data.total %>%
  ggplot(aes(x = IDS_pref,
             y = vocab_nwords,
             colour = CDI.agerange)) +
  facet_wrap(vars(fct_relevel(language_zone, "British", "NAE", "Other")),
             labeller = as_labeller(c("British" = "UK samples",
                                      "NAE" = "North American samples", "Other" = "Other"))) +
...
Here, we run a mixed-effects model including only theoretically motivated effects, as described in the pre-registration. We start with the full model below, simplifying the random effects structure until it converges.
