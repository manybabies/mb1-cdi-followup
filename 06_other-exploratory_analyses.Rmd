...
# Other values for other exclusion criteria- exploratory/not used

icctable_excl4 <- ICC(mb1_excl4_icc, missing = FALSE, lmer = TRUE) #ICC3k = .144
icc3k_excl4 <- icctable_excl4$results %>% filter(type == "ICC3k") %>% select("ICC") %>% pull()

icctable_excl6 <- ICC(mb1_excl6_icc, missing = FALSE, lmer = TRUE) #ICC3k = .142
icc3k_excl6 <- icctable_excl6$results %>% filter(type == "ICC3k") %>% select("ICC") %>% pull()

icctable_excl8 <- ICC(mb1_excl8_icc, missing = FALSE, lmer = TRUE) #ICC3k = .205
...
# Previous research suggests that CDI test-retest reliability is .86-.90 (Dale et al., 1989, Jahn-Smilo et al., 2001; Simonsen et al., 2014)
...
n_excl2 <- nrow(mb1_excl2_icc) # n = 467
n_excl4 <- nrow(mb1_excl4_icc) # n = 361
n_excl6 <- nrow(mb1_excl6_icc) # n = 293
n_excl8 <- nrow(mb1_excl8_icc) # n = 142



# Observed correlations we can detect at 80% power with each sample size under different exclusion criteria
pwr_excl2 <- round(pwr.r.test(n = n_excl2, power = .8)$r,2) # .13
pwr_excl4 <- round(pwr.r.test(n = n_excl4, power = .8)$r,2) # .15
pwr_excl6 <- round(pwr.r.test(n = n_excl6, power = .8)$r,2) # .16
pwr_excl8 <- round(pwr.r.test(n = n_excl8, power = .8)$r,2) # .23

# True correlations that can be detected given ICC associated with each exclusion criterion, assuming CDI reliability of .86
spearman_brown_true(pwr_excl2, .86, icc3k_excl2) # r = .398
spearman_brown_true(pwr_excl4, .86, icc3k_excl4) # r = .425
spearman_brown_true(pwr_excl6, .86, icc3k_excl6) # r = .457
spearman_brown_true(pwr_excl8, .86, icc3k_excl8) # r = .548

# True correlations that can be detected given ICC associated with each exclusion criterion, assuming CDI reliability of .95
spearman_brown_true(pwr_excl2, .95, icc3k_excl2) # r = .387
spearman_brown_true(pwr_excl4, .95, icc3k_excl4) # r = .405
spearman_brown_true(pwr_excl6, .95, icc3k_excl6) # r = .435
spearman_brown_true(pwr_excl8, .95, icc3k_excl8) # r = .521
