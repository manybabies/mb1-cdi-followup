...
full <- glmmTMB(vocabulary_percentile ~ z.ids_pref * z.test_age + z.ids_pref * cdi_age + z.ids_pref * protocol + z.ids_pref * nae + gender + (1 | lab) + (1 | participant), family = beta_family(link = "logit"), data=data)
...
reduced_model <- glmmTMB(vocabulary_percentile ~ z.ids_pref + z.test_age + cdi_age + protocol + nae + gender + (1 | lab) + (1 | participant), family = beta_family(link = "logit"), data=data)

lrtest(full, null)
```

```{r plots, include = TRUE, warning= FALSE}
ggplot(data = data, aes(x = gender , y = vocabulary_percentile, color = cdi.age_range)) +
  geom_point() +
  geom_boxplot() +
  facet_wrap(.~ lab)

ggplot(data = data, aes(x = gender , y = vocab_nwords, color = cdi.age_range)) +
  geom_point()+
  geom_boxplot() +
  facet_wrap(.~ lab)
```
