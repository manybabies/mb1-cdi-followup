...
AST: we should not use z_vocab_nwords as the DV for the UK model as in the RR, we stated that we will use raw vocab for non-NAE model.
```{r full_lmer}
# Run models =====================================
## NAE
...
# CDI.z_age_months
#==========
# Get printable results summary
(lmer.coef.nae <- lmer.full.nae %>%
    summary() %>%
    pluck("coefficients") %>%
    as_tibble(rownames = "Parameter") %>%
    select(-c("t value", "Pr(>|t|)")))
...
lmer.full.uk <- lmer(vocab_nwords ~ CDI.z_age_months + gender +
                       ...
                       IDS_pref:method + IDS_pref:CDI.z_age_months + IDS_pref:z_age_months +
                       (1 | labid),
                     data = data.uk) #AST: change it back to raw number of vocab words
#==========
# Sequentially removed random effects:
# IDS_pref:z_age_months
# IDS_pref:CDI.z_age_months
...
We wanted to compute *p*-vlues from model comparisons via likelihood ratio tests. The sequential term deletion and likelihood ratio tests are encapsulated in a helper function. However, model comparison led to singularity issues, thus we instead use a Type I anova with Satterthwaitte#'s method for ddfs from the `lmerTest` package.

```{r anova_lmer}
# Model comparison dropped from the analysis
check_lrtest <- F
if(check_lrtest){
  lmer.lr.nae <- lr_tests.merMod(lmer.full.nae)
  lmer.lr.uk <- lr_tests.merMod(lmer.full.uk)
}
# Get p-values with lmerTest::anova()
(lmer.aov.nae <- anova(lmer.full.nae, type = "III"))
(lmer.aov.uk <- anova(lmer.full.uk, type = "III"))
```
...
#==========
# Get printable results summary
(lmer.coef.uk_nae <- lmer.full.uk_nae %>%
    summary() %>%
    pluck("coefficients") %>%
    as_tibble(rownames = "Parameter") %>%
    select(-c("t value", "Pr(>|t|)")))
...
(lmer.aov.uk_nae <- anova(lmer.full.uk_nae, type = "III"))
...
if(run_models){
  ...
  # TODO: update lr_tests.brmsfit function to allow for automated Type III comparisons
  #bf.nae <- lr_tests.brmsfit(brm.full.nae) # Runs a sequential (Type I) model comparison
  ## Initialise bf list and parameters lists
  bf.nae <- list()
  two_ways.nae <- list("IDS_pref:method", "IDS_pref:CDI.z_age_months", "IDS_pref:z_age_months")
  one_ways.nae <- list("CDI.z_age_months", "gender", "z_age_months", "method", "IDS_pref")
  ## Two-way interactions
  bridge.full.nae <- bridge_sampler(brm.full.nae, silent = T)
  for(term in two_ways.nae){
    brm.two_way.nae <- update(brm.full.nae, formula(paste("~ . -", term)))
    bridge.two_way.nae <- bridge_sampler(brm.two_way.nae, silent = T)
    bf.nae[term] <- bayes_factor(bridge.full.nae, bridge.two_way.nae)$bf
  }
  ## One-way interactions
  brm.two_way.nae <- update(brm.full.nae,
                            ~ . - IDS_pref:method - IDS_pref:CDI.z_age_months - IDS_pref:z_age_months)
  bridge.two_way.nae <- bridge_sampler(brm.two_way.nae, silent = T)
  for(term in one_ways.nae){
    brm.one_way.nae <- update(brm.two_way.nae, formula(paste("~ . -", term)))
    bridge.one_way.nae <- bridge_sampler(brm.one_way.nae, silent = T)
    bf.nae[term] <- bayes_factor(bridge.two_way.nae, bridge.one_way.nae)$bf
  }
  # Save full model and Bayes factors
  bf.nae %<>% unlist() %>% as_tibble(rownames = "m_name")
...
if(run_models){
  # Run the full model
  brm.full.uk <- brm(vocab_nwords ~ CDI.z_age_months + gender + z_age_months +
                        method + IDS_pref + IDS_pref:method +
                        IDS_pref:CDI.z_age_months + IDS_pref:z_age_months +
                        (1 | labid),
  ...
  ## Initialise bf list and parameters lists
  bf.uk <- list()
  two_ways.uk <- list("IDS_pref:method", "IDS_pref:CDI.z_age_months", "IDS_pref:z_age_months")
  one_ways.uk <- list("CDI.z_age_months", "gender", "z_age_months", "method", "IDS_pref")
  ## Two-way interactions
  bridge.full.uk <- bridge_sampler(brm.full.uk, silent = T)
  for(term in two_ways.uk){
    brm.two_way.uk <- update(brm.full.uk, formula(paste("~ . -", term)))
    bridge.two_way.uk <- bridge_sampler(brm.two_way.uk, silent = T)
    bf.uk[term] <- bayes_factor(bridge.full.uk, bridge.two_way.uk)$bf
  }
  ## One-way interactions
  brm.two_way.uk <- update(brm.full.uk,
                            ~ . - IDS_pref:method - IDS_pref:CDI.z_age_months - IDS_pref:z_age_months)
  bridge.two_way.uk <- bridge_sampler(brm.two_way.uk, silent = T)
  for(term in one_ways.uk){
    brm.one_way.uk <- update(brm.two_way.uk, formula(paste("~ . -", term)))
    bridge.one_way.uk <- bridge_sampler(brm.one_way.uk, silent = T)
    bf.uk[term] <- bayes_factor(bridge.two_way.uk, bridge.one_way.uk)$bf
  }
  # Save full model and Bayes factors
  bf.uk %<>% unlist() %>% as_tibble(rownames = "m_name")
...
if(run_models){
  ...
  ## Initialise bf list and parameters lists
  bf.uk_nae <- list()
  two_ways.uk_nae <- list("IDS_pref:language_zone", "IDS_pref:method",
                          "IDS_pref:CDI.z_age_months", "IDS_pref:z_age_months")
  one_ways.uk_nae <- list("CDI.z_age_months", "language_zone",
                          "gender", "z_age_months", "method", "IDS_pref")
  ## Two-way interactions
  bridge.full.uk_nae <- bridge_sampler(brm.full.uk_nae, silent = T)
  for(term in two_ways.uk_nae){
    brm.two_way.uk_nae <- update(brm.full.uk_nae, formula(paste("~ . -", term)))
    bridge.two_way.uk_nae <- bridge_sampler(brm.two_way.uk_nae, silent = T)
    bf.uk_nae[term] <- bayes_factor(bridge.full.uk_nae, bridge.two_way.uk_nae)$bf
  }
  ## One-way interactions
  brm.two_way.uk_nae <- update(brm.full.uk_nae,
                               ~ . - IDS_pref:language_zone - IDS_pref:method -
                                 IDS_pref:CDI.z_age_months - IDS_pref:z_age_months)
  bridge.two_way.uk_nae <- bridge_sampler(brm.two_way.uk_nae, silent = T)
  for(term in one_ways.uk_nae){
    brm.one_way.uk_nae <- update(brm.two_way.uk_nae, formula(paste("~ . -", term)))
    bridge.one_way.uk_nae <- bridge_sampler(brm.one_way.uk_nae, silent = T)
    bf.uk_nae[term] <- bayes_factor(bridge.two_way.uk_nae, bridge.one_way.uk_nae)$bf
  }
  # Save full model and Bayes factors
  bf.uk_nae %<>% unlist() %>% as_tibble(rownames = "m_name")
