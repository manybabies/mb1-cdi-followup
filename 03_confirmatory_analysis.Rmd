...
## NAE
data.nae <- data.nae %>% 
  filter(!is.na(standardized.score.CDI.num)) %>% 
  mutate(log_standardized_score_CDI_num = log(standardized.score.CDI.num))
...
#### Checking mixed-model assumptions. We will check the following:
1) Linearlity
...
plot(robust_lmer.full.nae) #plot_model doesn't work with robustlmm, so we used plot(base)
...
plot_model(lmer.full.uk, type = 'diag')
```
...
```
#### Checking mixed-model assumptions
1) Linearlity
2) Normality of the residuals
3) Homoscedasticity of residuals
4) No autocorrelation
