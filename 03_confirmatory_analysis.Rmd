...
plot.age_group <- data.total %>%
  ...
  facet_wrap(vars(language_zone),
             labeller = as_labeller(c("British" = "UK sample",
                                      "NAE" = "North American sample"))) +
  xlab("IDS prefence score") + ylab("Vocabulary size (in words)") + theme(legend.position = "top") +
  geom_point() +
  geom_smooth(method = lm) +
...
```{r combined_lmer}
lmer.full.combined <- lmer(standardized.score.CDI.num ~ CDI.z_age_months + language_zone + gender +
