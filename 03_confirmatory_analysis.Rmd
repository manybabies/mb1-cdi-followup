...
# Run models =====================================
## NAE
lmer.full.nae <- lmer(z_standardized_CDI ~ gender + CDI.agerange + CDI.z_age_months +
                        CDI.agerange:CDI.z_age_months + IDS_pref + IDS_pref:method +
                        IDS_pref:CDI.agerange + IDS_pref:CDI.z_age_months +
                        IDS_pref:z_age_months + IDS_pref:CDI.agerange:CDI.z_age_months +
                        (1 + gender + CDI.agerange + CDI.z_age_months +
                        CDI.agerange:CDI.z_age_months + IDS_pref + IDS_pref:method +
                        IDS_pref:CDI.agerange + IDS_pref:CDI.z_age_months +
                        IDS_pref:z_age_months + IDS_pref:CDI.agerange:CDI.z_age_months | labid),
                      data = data.nae)
# Sequentially removed random effects:
# [list here]
## UK
lmer.full.uk <- lmer(z_vocab_nwords ~ gender + CDI.agerange + CDI.z_age_months +
                        CDI.agerange:CDI.z_age_months + IDS_pref + IDS_pref:method +
                        IDS_pref:CDI.agerange + IDS_pref:CDI.z_age_months +
                        IDS_pref:z_age_months + IDS_pref:CDI.agerange:CDI.z_age_months +
                        (1 + gender + CDI.agerange + CDI.z_age_months +
...
lmer.pwr.nae <- lmer.lr.nae %>%
  ...
  subset(`Pr(>Chisq)` < .05) %>%
  mutate(pwr = powerSim(lmer.lr.nae, test = fixed(Parameter, method = "lr")) %>% {.$x/.$n})
lmer.pwr.uk <- lmer.lr.uk %>%
  rownames_to_column(var = "Parameter") %>%
  subset(`Pr(>Chisq)` < .05) %>%
  mutate(pwr = powerSim(lmer.full.uk, test = fixed(Parameter, method = "lr")) %>% {.$x/.$n})
```
