...
* standardized scores incorrectly reported as numerical ranges instead of numbers, treated as NAs for the analysis, but kept in a separate column as characters and in an unprocessed data file (`data/cdi/unprocessed_merge.csv`); further, standardized score reported as `<1` were treated as zeros rather than NAs.
...
Cases in which we have two data points but a single unique value for `vocab_nwords` are pure duplicate entries. Checking by hand, we realise that the two duplicate data points for `babylablmu` result from a first entry with an error, and a second valid entry. However, the duplicate for `infantcogubc` seems to be constituted of two distinct measures with no indication of an error that would allow us to discard one of them. They might instead result from an incorrect `subid` for one of the two entries.

Although the scope of this script is not to exclude data points, we remove the pure duplicates and `babylablmu` "error duplicates" here while we know about them:

```{r cdi_fix_babylablmu, message=FALSE, warning=FALSE}
data.cdi <- data.cdi %>%
  subset(!(subid_unique %in% c("babylab-lmu:35", "babylab-lmu:76") &
             CDI.error == "error")) %>%
  unique()
```

However, the duplicate for `infantcogubc` seems to be constituted of two distinct measures with no indication of an error that would allow us to discard one of them. They might instead result from an incorrect `subid` for one of the two entries.
...
* Similarly for `babylabbrookes`, those were infants that were not included in the IDS analysis (see [issue #3](https://github.com/manybabies/mb1-cdi-followup/issues/3)). Notably participants `mb43` in our dataset should have been `mb143`, but this does not matter given that we do not keep their data.
