...
* standardized scores incorrectly reported as numerical ranges instead of numbers, treated as NAs for the analysis, but kept in a separate column as characters and in an unprocessed data file (`data/cdi/unprocessed_merge.csv`); further, standardized score reported as `<1` were treated as zeros rather than NA.
...
However, the duplicate for `infantcogubc` seems to be constituted of two distinct measures with no indication of an error that would allow us to discard one of them. This was checked with the lab, this was one participants that filled in the web questionnaire twice; here, we decide to keep the first entry, with `vocab_nwords == 289` (see [issue #7](https://github.com/manybabies/mb1-cdi-followup/issues/7)).

Although the scope of this script is not to exclude data points, we remove the pure duplicates and `babylablmu` and `unlv-musiclab` "error duplicates" here while we know about them:
...
data.cdi <- data.cdi %>%
  ...
  subset(!(subid_unique == "InfantCog-UBC:tp13" & vocab_nwords != 289)) %>%
  unique()
```
...
* For `unlvmusiclab` the only mismatch stems from the participant having beed a pilot infant for the IDS analysis. We correct its subiid here for consistency, but this participant will be exclude in script 02 (see [issue #6](https://github.com/manybabies/mb1-cdi-followup/issues/6)).
