...
df.final <- df.final %>%
  ...
  mutate(mean_IDS_pref = mean(IDS_pref),
         mean_LT = mean(mean_pair),
         z_IDS_pref = mean_IDS_pref / mean_LT) %>%
  ungroup() %>%
  select(-c(stimulus_num, IDS, ADS, IDS_pref, mean_pair, mean_IDS_pref, mean_LT)) %>%
  unique()
```
...
```{r z_age_days}
df.final <- df.final %>%
  mutate(z_age_months = scale(age_days, scale = F)/365*12,
         CDI.z_age_months = scale(CDI.agedays, scale = F)/365*12)
```

## `z_standardised_CDI`, `z_vocab_nwords`

Centering and scaling is good for DVs too! (`TODO`)
